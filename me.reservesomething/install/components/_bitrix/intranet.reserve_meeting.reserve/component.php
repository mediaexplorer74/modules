<?
//  Intranet.Reserve_Meeting.Reserve
//  v 2.0
//
//  Created: 15-jul-2020
//
//  Notes: Run BP when Meeting reserved =)
...
if (StrLen($arResult["FatalError"]) <= 0)
{
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && StrLen($_POST["save"]) > 0 && check_bitrix_sessid())
	{
		...
		if (StrLen($errorMessage) <= 0)
		{
			...
			// проверка режима --  ADD или UPDATE ?
			if ($arItem)
			{
				// Update Element Mode
				// res - result of Update  operation
				$res = $iblockElementObject->Update($arItem["ID"], $arFields);
				
				// сюда погружаем ид модифицируемого элемента
				$documentId = $arItem["ID"];
				
				$operation_mode = "UPDATE"; // режим - UPDATE
			...
			else
			{
				...
				// $idTmp - element id after Add operation 
				$idTmp = $iblockElementObject->Add($arFields);
				
				// сюда погружаем ид созданного элемента
				$documentId = $idTmp;
				
				$operation_mode = "ADD"; // режим - ADD
				
				// res - result of Add operation
				$res = ($idTmp > 0);
			}
			...
			else
			{
				...
				if (\Bitrix\Main\Loader::includeModule("lists")) 
				{
					...
					{
						// считываем из настроек модуля ид шаблона БП при добавлении элемента 
						//$BP_ID = COption::GetOptionInt('me.reservemeeting', 'WF_ID');
						$BP_ID = COption::GetOptionInt('me.reservemeeting', 'WF_ID_CREATE');
					}
					else
					{
						// считываем из настроек модуля ид шаблона БП при модификации элемента 
						$BP_ID = COption::GetOptionInt('me.reservemeeting', 'WF_ID_MODIFY');
					}
					
					
					// считываем ид инфоблока из настроек модуля
					...
					$arrParams['EVENT_ID'] = $documentId;
					$arrParams['AUTHOR_ID'] = $USER->GetID();
					$arrParams['OPERATION_MODE'] = $operation_mode; // ADD или UPDATE
					$arrParams['RESERVE_MEETING'] = "Y"; // признак, что именно переговорку разерезвировали!
					
					// Стартуем БП!
					// Передаем в него 4 параметра -- EVENT_ID, AUTHOR_ID, OPERATION_MODE, RESERVE_MEETING	
					$bpstart_res = CBPDocument::StartWorkflow(
								intval($BP_ID), array("iblock", "CIBlockDocument", $documentId), 
								$arrParams,//$arBizProcParametersValues, 
								$arErrorsTmp
					);
				}
