<?
//  Intranet.Reserve_Meeting.Meering
//  v 2.0
//
//  Created: 15-jul-2020
//
//  Notes: Run BP when Meeting modifyed (deleted) =)
...
if (StrLen($arResult["FatalError"]) <= 0)
{
	// ид эвента , который надо очистить
	...
	if ($clearId > 0 && check_bitrix_sessid())
	{
		...
		$dbElements = CIBlockElement::GetList(
			...
			array(
				...
				"SECTION_ID" => $arMeeting["ID"], // нужный раздел
				"ID" => $clearId, // нужный элемент
			),
			false,
			false,
		...
		if ($arElement = $dbElements->GetNext())
		{
			...
			)
			{
				...
				$operation_mode = "DELETE"; // режим - DELETE
				
				// считываем из настроек модуля ид шаблона БП при удалении элемента 
				$BP_ID = COption::GetOptionInt('me.reservemeeting', 'WF_ID_DELETE');
				...
				$documentId = $arElement["ID"];
				$arrParams['EVENT_ID'] = $arElement["ID"]; 
